...
第一个参数，我们称为 setup 函数，第一次的 setup 函数会在**组件 attached 时执行**。后续每次 setState 改变一个状态，都会执行 setup 函数。
...
熟悉 react 的同学会在 useEffect 中进行数据请求：
...
和 react useRef 不同的是，这里**不能用于获取子组件实例**。但可以用于缓存值，每次页面渲染时（setup 执行），能直接获取到缓存的值。例如：
